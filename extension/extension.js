...
game.import("extension", function (lib, game, ui, get, ai, _status) {
	...
	jl_config.jlname = {
		...
		zhugeguo: {
			...
			skill: {
				...
				"【羽化】":
					"▶结束阶段，你有95%的概率观看牌堆顶X张牌（X为你本回合使用的锦囊牌数且至少为4），然后保留其中至多三张牌。若这三张牌花色均不同，你随机对与你阵营不同的一名其他角色造成1-2点伤害。",
			},
		},
		nianshou: {
			name: "年兽",
		...
		//   "des": "",
		//   "skill": {
		//     "【气霸】": "▶当你造成或受到伤害后，你有80%的概率可以令至多两名角色各摸一张牌（每回合限3次）。",
		//     "【坚韧】": "▶当你进入濒死状态时，你有90%的概率回复体力至4点并摸三张牌。（每局游戏限一次）"
		//   }
		// },
		// "sunce": {
		//   "name": "孙策",
		//   "des": "",
		//   "skill": {
		//     "【制霸】": "▶结束阶段，你有90%的概率可以和一名其他角色拼点。若你赢，你随机获得其一张手牌且视为你对其使用一张【决斗】；若你没赢，你获得双方拼点的牌并摸一张牌。",
		//     "【激昂】": "▶当你使用或被使用伤害类锦囊牌或红色【杀】后（指定或成为目标后），你有85%的概率摸1~2张牌（每回合限两次）。"
		//   }
		// },
		sunshangxiang: {
			name: "孙尚香",
		...
		//   "des": "",
		//   "skill": {
		//     "【国色】": "▶出牌阶段开始时，你有85%的概率可摸一张方块牌并可以此牌当【乐不思蜀】使用，且可以弃置场上一张【乐不思蜀】。",
		//     "【流离】": "▶成为【杀】的目标后，你有85%的概率可弃置一张牌将此【杀】转移给你攻击范围内的一名其他角色并摸一张牌（不能是【杀】的使用者，每回合限触发两次）。"
		//   }
		// },
		jiangwei: {
			name: "姜维",
	...
	jl_config.settexiao = function (type) {
		if (
			(lib.config.jl_juneichangzhu == 1) &
			(lib.config.jl_gmczjl != 0) &
			(lib.config.jl_gmczjl !== "shenlvbu")
		) {
			let img = document.createElement("img");
			img.id = "jl_texiao";
	...
	jl_config.chooseButton = function (skills, player) {
		...
		const dialog = ui.create.dialog(
			"请为" + get.translation(player) + "选择将灵",
			"",
			"hidden"
		);
		event.dialog = dialog;
					...
					e => {
						...
						node.addEventListener(
							...
							e => {
								...
								winDialog.style.transition =
									"all 0.5s cubic-bezier(0, 0, 0.2, 1) 0s";
								winDialog.style.transform = winDialog.style.transform.replace(
									/translateX\(\d+(px)?\)/,
									"translateX(-140px)"
								);
								node.addEventListener(
		...
		constructor() {
				...
				if (lib.config.jl_value[i]) {
					if (lib.config.jl_value[i]["quality"] === "s") {
						...
						if (lib.config.jl_value[i]["gain"]) {
							jl.style.backgroundImage =
								"url(" +
								jl_config["layoutPath"] +
								"image/character/" +
								i +
								".png" +
								")";
			...
			for (let i of this.jl_character) {
				i.addEventListener("click", () => {
					...
					document.getElementById("pattern").style.backgroundImage =
						"url(" + jl_config["layoutPath"] + "image/action/" + name + "2.gif" + ")";
					this.detail.show();
			...
			this.kaichangbai = ui.create.div(
				".kaichangbai",
				this.pmj,
				"这里 不是你该来的地方... ..."
			);
			...
			this.pmj2.id = "pmj2";
			this.pmj2.hide();
			this.kaichangbai2 = ui.create.div(
				".kaichangbai2",
				this.pmj2,
				"这是哪里？我怎么会在这儿？将灵和那股神秘的力量开始不稳定起来，再往前走可能会无法回头了... ..."
			);
			this.kuaipao = ui.create.div(".likai", this.pmj2, "快跑");
			this.zhong = ui.create.div(".jietuo", this.pmj2, "向前");
			let beizhan = lib.config.jl_gmczjl;
			...
			this.queding1.addEventListener("click", () => {
				...
				this.tips.hide();
				this.shadow1.style.backgroundImage =
					"url(" + jl_config["layoutPath"] + "image/action/" + beizhan + "1.gif" + ")";
				this.shadow1.show();
				setTimeout(function () {
					game.playThisAudio("effect/zhuansheng");
					this.shadow1.style.backgroundImage =
						"url(" + jl_config["layoutPath"] + "image/action/" + name + "1.gif" + ")";
					this.shadow2.style.backgroundImage =
						"url(" + jl_config["layoutPath"] + "image/img/zhuansheng1.gif" + ")";
					this.shadow3.style.backgroundImage =
						"url(" + jl_config["layoutPath"] + "image/img/reward1.png" + ")";
					this.shadow2.show();
					this.shadow3.show();
				}, 500);
				setTimeout(function () {
					this.shadow3.style.backgroundImage =
						"url(" + jl_config["layoutPath"] + "image/img/reward2.png" + ")";
				}, 700);
				setTimeout(function () {
					this.shadow2.hide();
				}, 1000);
			...
			this.jietuo.addEventListener("click", () => {
				...
				document.getElementById("kaichangbai").innerHTML =
					"一切都已结束 你可以离开了... ...";
			...
			this.qlin.addEventListener("click", () => {
				document.getElementById("pmj").style.backgroundImage =
					"url(" + jl_config["layoutPath"] + "image/img/qlin.jpg" + ")";
			...
			this.ql.addEventListener("click", () => {
				document.getElementById("pmj").style.backgroundImage =
					"url(" + jl_config["layoutPath"] + "image/img/ql.jpg" + ")";
				this.kaichangbai.hide();
				this.rukou.hide();
				const a = lib.config.jl_gmczjl;
			...
			this.bh.addEventListener("click", () => {
				document.getElementById("pmj").style.backgroundImage =
					"url(" + jl_config["layoutPath"] + "image/img/bh.jpg" + ")";
				this.kaichangbai.hide();
				this.rukou.hide();
				const a = lib.config.jl_gmczjl;
			...
			this.zq.addEventListener("click", () => {
				document.getElementById("pmj").style.backgroundImage =
					"url(" + jl_config["layoutPath"] + "image/img/zq.jpg" + ")";
				this.kaichangbai.hide();
				this.rukou.hide();
				const a = lib.config.jl_gmczjl;
			...
			this.xw.addEventListener("click", () => {
				document.getElementById("pmj").style.backgroundImage =
					"url(" + jl_config["layoutPath"] + "image/img/xw.jpg" + ")";
				this.rukou.hide();
				this.kaichangbai.hide();
				const a = lib.config.jl_gmczjl;
			...
			this.zhong.addEventListener("click", () => {
				document.getElementById("pmj2").style.backgroundImage =
					"url(" + jl_config["layoutPath"] + "image/img/wszl.jpg)";
		...
		constructor() {
			...
			this.hl_button1.addEventListener("click", () => {
				...
				if (parseInt(lib.config.jl_hljh) > 0) {
					...
					this.daoju.style.backgroundImage =
						"url(" +
						jl_config["layoutPath"] +
						"image/sp/" +
						"sp_" +
			...
			this.lantern2.addEventListener("dblclick", () => {
				game.playThisAudio("effect/click");
				this.tishis = ui.create.div(
					".tishi",
					this.hl,
					"作弊成功，获得唤灵锦盒*50、转生丹*50、高级转生丹*50"
				);
		...
		constructor() {
			...
			for (var i = 0; i < commodity.length; i++) {
				...
				if (ccommodity.includes(commodity[i])) {
					this.spnumber = ui.create.div(
						".spnumber",
						this.sell,
						"(" + lib.config.jl_hljh + "/1)"
					);
					...
					this.spnumber = ui.create.div(
						".spnumber",
						this.sell,
						"(" + lib.config.jl_value[commodity[i]]["sp"] + "/20)"
					);
				}
				this.spnumber.id = suojinumber[i];
				numberid[i] = this.spnumber.id;
				let soul = ui.create.div(".soul", this.sell);
				soul.style.backgroundImage =
					"url(" +
					jl_config["layoutPath"] +
					"image/sp/" +
					"sp_" +
				...
				i.addEventListener("click", () => {
					...
					if (!ccommodity.includes(i.dataset.buyname)) {
						if (lib.config.jl_value[i.dataset.buyname]["sp"] > 19) {
							lib.config.jl_value[i.dataset.buyname]["sp"] =
								lib.config.jl_value[i.dataset.buyname]["sp"] - 20;
							this.spnumber.innerHTML =
								"(" + lib.config.jl_value[i.dataset.buyname]["sp"] + "/20)";
							lib.config.jl_value[i.dataset.buyname]["gain"] = true;
							game.saveConfig("jl_value", lib.config.jl_value);
							document.getElementById(numberid[i.dataset.buynumber]).innerHTML =
								"(" + lib.config.jl_value[i.dataset.buyname]["sp"] + "/20)";

							game.playThisAudio("effect/zhuansheng");
							this.shadowss.show();
							this.shadow2ss.style.backgroundImage =
								"url(" +
								jl_config["layoutPath"] +
								"image/img/zhuansheng1.gif" +
								")";
							this.shadow3ss.style.backgroundImage =
								"url(" + jl_config["layoutPath"] + "image/img/reward1.png" + ")";
							this.shadow2ss.show();
							this.shadow3ss.show();
							setTimeout(() => {
							...
							setTimeout(() => {
								this.shadow1ss.style.backgroundImage =
									"url(" +
									jl_config["layoutPath"] +
									"image/action/" +
									i.dataset.buyname +
							...
							this.duihuan = ui.create.div(
								".buzu",
								this.shop,
								"碎片不足，是否用传说灵韵石直接兑换？(双击确定)"
							);
						...
						if (parseInt(lib.config.jl_hljh) > 0) {
							game.saveConfig("jl_hljh", parseInt(lib.config.jl_hljh - 1));
							this.shop_jinhe.innerHTML = lib.config.jl_lingzhu;
							document.getElementById(numberid[12]).innerHTML =
								"(" + lib.config.jl_hljh + "/1)";
							document.getElementById(numberid[11]).innerHTML =
								"(" + lib.config.jl_hljh + "/1)";
							document.getElementById(numberid[10]).innerHTML =
								"(" + lib.config.jl_hljh + "/1)";
							lib.config.jl_value[i.dataset.buyname]["gain"] = true;
							game.saveConfig("jl_value", lib.config.jl_value);
							this.shadowss.show();
						...
						zhanshen: {
							...
							content: function () {
								...
								player
									.chooseTarget(
										get.prompt(
											"指定任意数量目标，直到目标的回合结束前所有技能失效。使用后本回合你的攻击无视其防具且你造成的伤害+1。"
										),
										[1, Infinity],
								...
								if (result.bool) {
									...
									for (var i = 0; i < result.targets.length; i++)
										result.targets[i].addTempSkill("jl_shenlvbu_wq_a", {
											player: "phaseAfter",
										});
						...
						wq_b: {
							...
							filter: function (event, player) {
								return (
									event.source &&
									event.source == player &&
									event.player.hasSkill("jl_shenlvbu_wq_a")
								);
							},
							content: function () {
								trigger.num++;
						...
						shennu: {
							...
							content: function () {
								...
								player
									.chooseTarget(
										get.prompt(
											"指定任意数量目标，按指定的顺序，所有目标依次弃置所有牌，之后造成1点伤害。"
										),
										[1, Infinity],
										function (card, player, target) {
											return target != player;
							...
							content: function () {
								...
								var chooseButton = function (online, player, cards) {
													...
													for (
														...
														if (
															event.dialog.buttons[
																i
															].classList.includes("glow") == false &&
															event.dialog.buttons[
																i
															].classList.includes("target") == false
														)
															ui.cardPile.appendChild(
																event.dialog.buttons[i].link
															);
													}
													player.popup(
									...
									event.custom.replace.button = function (link) {
										var event = _status.event;
										if (link.classList.includes("target")) {
											link.classList.remove("target");
											event.top.remove(link);
										} else if (link.classList.includes("glow")) {
									...
									event.custom.replace.window = function () {
										for (
											var i = 0;
											i < _status.event.dialog.buttons.length;
											i++
										) {
								...
								if (event.result == "ai" || !event.result) {
									...
									for (i = 0; i < event.cards.length; i++) {
										if (
											!top.includes(event.cards[i]) &&
											!bottom.includes(event.cards[i])
										) {
									...
									player.popup(
										get.cnNumber(top.length) +
											"上" +
											get.cnNumber(event.cards.length - top.length) +
											"下"
									);
									game.log(
										player,
					...
					subSkill: {
						wushen1: {
							...
							content: function () {
								"step 0";
								player
									.chooseTarget(get.prompt("武神"), [1, 3])
									.set("ai", function (target) {
							...
							filter: function (event, player) {
								if (!event.num) {
									...
									return (
										...
										!player.storage.xinzhengnan.includes(event.player)
									);
								}
							},
							content: function () {
					...
					subSkill: {
						...
						shuli: {
							...
							content: function () {
								...
								console.log(`event.damage${event.damage}`);
								event.gainList = [];
								("step 1");
				...
				jl_shencaocao: {
					...
					subSkill: {
						...
						guixin: {
							...
							prompt: "结束阶段，你有95%的概率可以随机获得任意名角色每人一张手牌，然后这些角色再随机弃置一张装备区里的牌，因此失去最后一张手牌或装备区的牌的角色失去1点体力。",
							filter: function (event, player) {
								return Math.random() < 0.95;
							},
							content: function () {
								"step 0";
								player
									.chooseTarget(get.prompt("jl_shencaocao_guixin"))
									.set("ai", function (target) {
						...
						lingren: {
							...
							content: function () {
								...
								player
									...
									.set(
										"prompt2",
										"你使用【杀】或伤害类锦囊牌指定目标后，你选择其中一个目标令此牌对其伤害+1~2然后你摸1~3张牌，并且你获得“界奸雄”、“界行殇”直到你下回合开始。（每回合限触发2次）"
									);
					...
					subSkill: {
						qirang: {
							...
							content: function () {
								...
								if (_status.currentPhase != player)
									player.getStat().skill.jl_zhugeguo_qirang++;
							},
						},
						count: {
							forced: true,
						...
						yuhua: {
							...
							prompt2: function (event, player) {
								...
								return "观看牌堆顶" + numc + "张牌，然后保留其中至多三张牌";
							},
							...
							content: function () {
								...
								player
									.chooseCardButton(
										...
										true,
										event.cards,
										"请选择要保留的三张牌，然后若这三张牌花色均不同，你随机对与你阵营不同的一名其他角色造成1-2点伤害。"
									)
					...
					subSkill: {
						qinyin: {
							...
							content: function () {
								...
								("step 1");
								player
									.chooseTarget(get.prompt("jl_shenzhouyu_qinyin"), [1, Infinity])
									.set("ai", function (target) {
								...
								if (result.bool) {
									result.targets.sortBySeat();
									game.playThisAudio(
										"voice/" + event.name + [1, 2, 3].randomGet()
									);
									if (player == game.me) {
										jl_config.settexiao();
									}
					...
					subSkill: {
						...
						xiaoji: {
							...
							content: function () {
								...
								if (event.count > 0) {
									player
										.chooseBool(get.prompt2("jl_ssx_xiaoji"))
										.set("frequentSkill", "jl_ssx_xiaoji").ai = lib.filter.all;
								}
								("step 3");
								if (result.bool) {
				...
				jl_lvbu: {
					...
					subSkill: {
						wushuang: {
							...
							locked: true,
							prompt2:
								"令此杀不能被抵消且无视防具，且结算完成后有80%的概率回收此【杀】并获得一张决斗",
							filter: function (event, player) {
								var numa = Math.random();
					...
					subSkill: {
						...
						juxiang: {
							...
							content: function () {
								...
								if (player.hasSkill("jl_zhurong_count1"))
									player.addTempSkill("jl_zhurong_count2", "roundStart");
											...
											function (link) {
												...
												if (link == "ok") {
													if (online) {
														...
														for (var i = 0; i < event.top.length; i++) {
															event._result.top.push(
																event.top[i].link
															);
														}
														for (
															var i = 0;
															i < event.bottom.length;
															i++
														) {
															event._result.bottom.push(
																event.bottom[i].link
														...
														for (
															...
															if (
																event.dialog.buttons[
																	i
																].classList.includes("glow") ==
																	false &&
																event.dialog.buttons[
																	i
																].classList.includes("target") ==
																	false
															)
																ui.cardPile.appendChild(
																	event.dialog.buttons[i].link
						...
						yanli: {
							...
							filter: function (event, player) {
								...
								return (
									...
									!player.hasSkill("jl_xiaotao_count2") &&
									numa < 0.9
								);
							},
							trigger: { global: "dying" },
							content: function () {
								if (player.hasSkill("jl_xiaotao_count1"))
									player.addTempSkill("jl_xiaotao_count2", "roundStart");
	...
	for (const jlnameKey in jl_config.jlname) {
		...
		out_package.skill.translate[`jl_${jlnameKey}`] = jlName;
		out_package.skill.translate[`jl_${jlnameKey}_info`] = `将灵${jlName}，拥有技能\n`;
		for (const skillName in jl_config.jlname[jlnameKey].skill) {
			out_package.skill.translate[`jl_${jlnameKey}_info`] +=
				`<span style="color:#9933ff"><abbr title="${skillName}：${jl_config.jlname[jlnameKey].skill[skillName]}"><ins>${skillName}</ins></abbr></span>\n`;
		}
	}
	...
	return {
		...
		content: function () {
			...
			if (
				lib.config[jl_config.extensionName + "jl_gmjl"] != "off" &&
				lib.config.jl_gmczjl != undefined
			) {
				lib.skill._jl_gmjl = {
					trigger: {
						global: "gameStart",
							...
							if (game.players[event.jl_num] == game.me) {
								...
								if (game.me.side != undefined) {
									if (game.players[event.jl_num].side == game.me.side) {
										jl_config.chooseButton(
											event.jl_skill,
											game.players[event.jl_num]
										);
							...
							switch (event.jl_add) {
								case "same": {
									for (let i of gameplayer) {
										if (i == game.me) i.addSkill("jl_" + event.q);
										if (i.side != undefined) {
											if (i.side == friend) i.addSkill("jl_" + event.q);
										}
									}
									break;
								}
		...
		precontent: function (config) {
			...
			lib.skill._jl_hiddenskill = {
				...
				content: function () {
					...
					player
						.chooseButton([0, len])
						.set("createDialog", ["选择幻化的武将", [list, "character"]]);
					("step 1");
					if (result.bool) {
						if (result.links) {
				...
				content: function () {
					...
					if (lib.config.jl_gongji == 1 && trigger.source.storage.playerAnimation) {
						if (lib.config.jl_gmczjl === undefined && !jl_config.gongji_flag) {
							...
							let jl_skill = [];
							let hasjl = false;
							for (let i in jl_config.jlname) jl_skill.push("jl_" + i);
							jl_skill.forEach(value => {
								if (trigger.source.hasSkill(value)) hasjl = value;
							});
							if (hasjl) jl_config.damageAnimation(hasjl.slice(3), trigger, event);
